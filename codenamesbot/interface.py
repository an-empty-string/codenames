...
class Interface():
    ...
    def player_joins(self, player, team=None):
        if team is None:
            ...
            self.tell(f"{player} has joined the {team} team.")
    ...
    def team_guessed(self, actor, team, word):
        ...
        self.tell(f"{actor} has revealed a {team} agent. {remaining} left.")
    ...
    def spymaster_view(self):
        ...
        return f"Assassin: {self.game.assassin} | Green: {greens} | Pink: {pinks} | Civilians: {civilians}"
    ...
    def notify_guessing(self, team, guessers):
        ...
        self.tell(
            f"{joined}: You're up! It's {team}'s turn to guess. The clue is {hint[0]} ({number_str})."
        )
    ...
    def notify_winner(self):
        self.tell(f"The game is over. The {self.game.winner} team wins!")
